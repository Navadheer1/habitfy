...
// If we are not on port 5000 (e.g. using Live Server), point to the backend explicitly
const API_URL = (window.location.protocol === 'file:' || window.location.hostname === '' || (window.location.port !== '5000' && window.location.hostname === 'localhost'))
...
async function validateAndSyncAuthToken() {
    ...
    try {
        ...
        if (user.name) localStorage.setItem('authUserName', user.name);
        if (user.email) localStorage.setItem('authUserEmail', user.email);
    } catch (e) {
    ...
    try {
        ...
        if (data.status === 'ok') {
            if (data.db === 'connected') {
                ...
                el.title = 'Backend: Online, Database: Connected';
            } else {
                el.className = 'status-badge db-error';
                el.title = 'Backend: Online, Database: Disconnected';
            }
        } else {
        ...
        el.className = 'status-badge offline';
        el.title = 'Backend: Unreachable (Make sure server.js is running)';
    }
}

async function initApp() {
    updateBackendStatus();
    await validateAndSyncAuthToken();

    const token = localStorage.getItem('token');
    const persistedName = localStorage.getItem('authUserName');
    const persistedEmail = localStorage.getItem('authUserEmail');
    currentUser = sessionStorage.getItem(STORAGE_KEY_SESSION) || (token ? (persistedName || persistedEmail) : null);
    appData = normalizeAppData(await loadData());

    if (token) {
        const displayName = persistedName || currentUser;
        if (displayName) appData.user.username = displayName.toString();
...
async function loadData() {
    ...
    if (token) {
        try {
            ...
            let apiHabits = [];
            if (res.ok) {
                apiHabits = await res.json();
            }

            // Load local extra data (XP, Fitness)
            const userId = localStorage.getItem('userId');
            const localExtraKey = userId ? `extra_data_${userId}` : null;
            const localExtra = localExtraKey ? JSON.parse(localStorage.getItem(localExtraKey) || '{}') : {};

            // Merge
            const mergedData = { ...JSON.parse(JSON.stringify(defaultState)), ...localExtra };
            
            const habitMap = new Map();
            
            // 1. Start with local habits (definitions)
            if (Array.isArray(mergedData.habits)) {
                mergedData.habits.forEach(h => habitMap.set(h.name, h));
            }
            
            // 2. Process API habits
            apiHabits.forEach(apiH => {
                if (!habitMap.has(apiH.habitName)) {
                    // Create new definition if missing locally
                    habitMap.set(apiH.habitName, {
                ...
                const habit = habitMap.get(apiH.habitName);
                if (!habit.logs) habit.logs = {};
                habit.logs[apiH.date] = { status: apiH.status, _id: apiH._id };
            });
            
            mergedData.habits = Array.from(habitMap.values());
            
            return normalizeAppData(mergedData);
        } catch (e) {
            console.error(e);
...
function normalizeAppData(data) {
    ...
    base.plans = Array.isArray(src.plans) ? src.plans : [];
    ...
    base.plans = base.plans
        ...
        .map(p => {
            const id = (p.id || p._id || (crypto.randomUUID ? crypto.randomUUID() : 'p-' + Date.now() + Math.random().toString(36).slice(2))).toString();
            return {
                id,
                ...
                targetDate: (p.targetDate || '').toString(),
                tags: Array.isArray(p.tags) ? p.tags.map(t => (t || '').toString()).filter(Boolean) : [],
                contentHtml: (p.contentHtml || '').toString(),
                milestones: Array.isArray(p.milestones) ? p.milestones : [],
                steps: Array.isArray(p.steps) ? p.steps : [],
                updatedAt: (p.updatedAt || p.lastUpdatedAt || p.lastUpdated || '').toString()
            };
        });
...
async function fetchUserState() {
    try {
        ...
        if (res.ok) {
            ...
            appData.user.xp = Number(state.xp) || appData.user.xp;
            appData.user.level = Number(state.level) || appData.user.level;
            appData.user.rank = (state.rank || appData.user.rank || 'bronze').toString();
            appData.user.subscriptionPlan = state.subscriptionPlan || 'free';
            appData.user.planFeatures = state.planFeatures || {};
            updateUI();
        }
    } catch (e) {
...
function showUpgradePrompt(message) {
    alert(message || 'Upgrade to Pro to unlock this feature.');
}
...
async function onboardingChooseGoal(goal) {
    const token = localStorage.getItem('token');
    ...
    if (res.ok) {
        await onboardingLoadTemplates(goal);
        document.getElementById('onboardingStep1').style.display = 'none';
        document.getElementById('onboardingStep2').style.display = 'block';
    }
}

async function onboardingLoadTemplates(goal) {
    const token = localStorage.getItem('token');
    const res = await fetch(`${API_URL}/onboarding/templates?goal=${encodeURIComponent(goal)}`, {
        headers: { 'Authorization': `Bearer ${token}` }
    });
    if (res.ok) {
        const list = await res.json();
        const container = document.getElementById('onboardingTemplates');
        if (container) {
            container.innerHTML = '';
            list.forEach(t => {
                ...
                el.innerHTML = `
                  <input type="checkbox" name="obTemplate" value="${t._id}">
...
async function onboardingApplyTemplates() {
    const token = localStorage.getItem('token');
    const ids = Array.from(document.querySelectorAll('input[name="obTemplate"]:checked')).map(i => i.value);
    const res = await fetch(`${API_URL}/onboarding/habits`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
...
async function onboardingJoinChallenge() {
    const token = localStorage.getItem('token');
    const res = await fetch(`${API_URL}/onboarding/join-challenge`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
    });
    if (res.ok) {
        const previewRes = await fetch(`${API_URL}/onboarding/progress-preview`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        if (previewRes.ok) {
            const p = await previewRes.json();
            const el = document.getElementById('onboardingPreviewText');
            if (el) {
                el.innerText = `Projected XP in 30 days: ${p.projectedXp30d}`;
            }
        }
...
function ensureSocialUser(username) {
    ...
    if (!su.feed || typeof su.feed !== 'object') su.feed = { hidden: [] };
    if (!Array.isArray(su.feed.hidden)) su.feed.hidden = [];
...
function saveData() {
    ...
    if (token && userId) {
        localStorage.setItem(`extra_data_${userId}`, JSON.stringify(appData));
    } else if (currentUser) {
    ...
    const plansEl = document.getElementById('plansSection');
    ...
    const isPlansVisible = plansEl ? plansEl.style.display !== 'none' : false;
    ...
    if (isFriendsVisible && !isDashboardVisible && !isFitnessVisible) {
        renderFriends();
    } else if (isPlansVisible && !isDashboardVisible && !isFitnessVisible && !isFriendsVisible) {
        renderPlans();
...
function switchTab(tab) {
    ...
    if (plansEl) plansEl.style.display = tab === 'plans' ? 'block' : 'none';
...
function visibleFeedFor(viewer, scope, offset, limit) {
    ...
    const hidden = asSet(uv.feed?.hidden || []);
...
function renderFriendsNotifications() {
    ...
    el.innerHTML = items.length
        ? items.map((n, idx) => {
            ...
            const key = (n.id || '').toString() || `legacy:${idx}:${(n.createdAt || '').toString()}:${(n.text || '').toString()}`;
            return `<div class="friends-notif ${unread ? 'unread' : ''}"><button class="friends-notif-dismiss" type="button" data-action="notif_dismiss" data-notif-key="${key}">&times;</button><div style="display:flex; justify-content:space-between; gap:1rem; padding-right:1.4rem;"><div style="font-weight:800;">${n.text}</div><div class="time">${formatAgo(n.createdAt)}</div></div>${from}</div>`;
        }).join('')
...
function renderFeedItem(viewer, e) {
    ...
    return `
        <div class="friends-feed-item" data-event="${e.id}">
            <button class="friends-feed-dismiss" type="button" data-action="feed_dismiss" data-event="${e.id}">&times;</button>
            <div class="meta">
                <div class="actor">
                    <div class="friends-mini-avatar">${photo ? `<img src="${photo}" alt="">` : `<i class="fas fa-user"></i>`}</div>
                    <span>${e.actor}</span>
...
function setupFriendsEventListeners() {
    ...
    if (friendsSection && friendsSection.dataset.boundDelegation !== '1') {
        ...
        friendsSection.addEventListener('click', (e) => {
            ...
            const notifKey = btn.dataset.notifKey;

            if (action === 'feed_dismiss') {
                const su = ensureSocialUser(currentUser);
                if (!su) return;
                const id = (ev || '').toString();
                if (!id) return;
                su.feed.hidden = uniq([...(su.feed.hidden || []), id]);
                saveSocialStore();
                renderFriendsFeed(true);
                return;
            }
            ...
            if (action === 'notif_dismiss') {
                const su = ensureSocialUser(currentUser);
                if (!su) return;
                const list = Array.isArray(su.notifications?.items) ? su.notifications.items : [];
                const k = (notifKey || '').toString();
                su.notifications.items = list.filter((n, idx) => {
                    const nk = (n?.id || '').toString() || `legacy:${idx}:${(n?.createdAt || '').toString()}:${(n?.text || '').toString()}`;
                    return nk !== k;
                });
                saveSocialStore();
                renderFriendsNotifications();
...
function plansIdentityKey() {
    const token = localStorage.getItem('token');
    const userId = localStorage.getItem('userId');
    if (token && userId) return `auth:${userId}`;
    if (currentUser) return `local:${currentUser}`;
    return 'guest';
}

function plansIsAuthed() {
    const token = localStorage.getItem('token');
    const userId = localStorage.getItem('userId');
    return !!(token && userId);
}

function plansSetSaveIndicator(text) {
    const el = document.getElementById('plansSaveIndicator');
    if (!el) return;
    el.innerText = text;
}

function plansSchedule(key, ms, fn) {
    const prev = plansState.debouncers.get(key);
    if (prev) clearTimeout(prev);
    const t = setTimeout(() => {
        plansState.debouncers.delete(key);
        fn();
    }, ms);
    plansState.debouncers.set(key, t);
}

function plansGetById(id) {
    const k = (id || '').toString();
    return plansState.items.find(p => (p?.id || '').toString() === k) || null;
}

function plansGetSelected() {
    return plansGetById(plansState.selectedId);
}

function plansNormalizeApiPlan(p) {
    const id = (p?._id || p?.id || '').toString();
    const tags = Array.isArray(p?.tags) ? p.tags.map(t => (t || '').toString()).filter(Boolean) : [];
    return {
        id,
        title: (p?.title || '').toString(),
        shortDescription: (p?.shortDescription || '').toString(),
...
async function plansApi(path, options = {}) {
    const token = localStorage.getItem('token');
    ...
    if (!res.ok) {
        const msg = (data && typeof data === 'object' && (data.msg || data.message)) ? (data.msg || data.message) : 'Request failed';
        throw new Error(msg);
    }
    return data;
}

function plansComputeProgress(plan) {
...
function plansRenderList() {
    const el = document.getElementById('plansList');
    if (!el) return;

    const q = (document.getElementById('plansSearchInput')?.value || '').toString().trim().toLowerCase();
    const fCat = (document.getElementById('plansFilterCategory')?.value || 'all').toString();
    const fPri = (document.getElementById('plansFilterPriority')?.value || 'all').toString();
    const fStatus = (document.getElementById('plansFilterStatus')?.value || 'all').toString();

    const filtered = (plansState.items || []).filter(p => {
        if (!p) return false;
        if (fCat !== 'all' && (p.category || '') !== fCat) return false;
        if (fPri !== 'all' && (p.priority || '') !== fPri) return false;
        if (fStatus !== 'all' && (p.status || '') !== fStatus) return false;
        if (q) {
            const hay = `${p.title || ''} ${(p.shortDescription || '')} ${(p.contentHtml || '')}`.toLowerCase();
    ...
    el.innerHTML = filtered.length
        ? filtered.map(p => {
            const pr = plansComputeProgress(p);
            const title = (p.title || 'Untitled').toString();
            const sub = `${p.category || 'Other'} • ${p.priority || 'Medium'} • ${p.status || 'Active'}`;
            const active = (p.id || '').toString() === (plansState.selectedId || '').toString();
            return `
                <div class="plans-list-item ${active ? 'active' : ''}" data-action="plans_select" data-plan="${p.id}">
                    <div style="min-width:0;">
                        <div class="title">${escapeHtml(title)}</div>
                        <div class="sub">${escapeHtml(sub)}</div>
                    </div>
                    <div class="right">
                        <div class="plans-mini-progress"><div class="fill" style="width:${pr.pct}%"></div></div>
                    </div>
                </div>
            `;
        }).join('')
        : '<div class="hint">No plans yet</div>';
}

function plansRenderSelected() {
    const plan = plansGetSelected();
    const empty = document.getElementById('plansEmptyState');
    const wrap = document.getElementById('plansEditorWrap');
    const title = document.getElementById('plansTitleInput');
    const shortDesc = document.getElementById('plansShortDescInput');
    const editor = document.getElementById('plansEditor');

    const catSel = document.getElementById('plansEditCategory');
    const priSel = document.getElementById('plansEditPriority');
    const statusSel = document.getElementById('plansEditStatus');
    const tagsInput = document.getElementById('plansEditTags');
    const startInput = document.getElementById('plansEditStartDate');
    const targetInput = document.getElementById('plansEditTargetDate');
    ...
    if (!plan) {
        ...
        plansSetSaveIndicator('Saved');
        const last = document.getElementById('plansLastUpdated');
        if (last) last.innerText = '';
        const badges = document.getElementById('plansMetaBadges');
        if (badges) badges.innerHTML = '';
        const fill = document.getElementById('plansProgressFill');
        const txt = document.getElementById('plansProgressText');
        if (fill) fill.style.width = '0%';
        if (txt) txt.innerText = '0/0 • 0%';
        const stepsEl = document.getElementById('plansStepsList');
        const msEl = document.getElementById('plansMilestones');
    ...
    if (title && title.value !== plan.title) title.value = plan.title || '';
    if (shortDesc && shortDesc.value !== plan.shortDescription) shortDesc.value = plan.shortDescription || '';
    if (editor && editor.innerHTML !== (plan.contentHtml || '')) editor.innerHTML = plan.contentHtml || '';

    if (catSel) catSel.value = plan.category || 'Other';
    if (priSel) priSel.value = plan.priority || 'Medium';
    if (statusSel) statusSel.value = plan.status || 'Active';
    if (tagsInput) tagsInput.value = Array.isArray(plan.tags) ? plan.tags.join(', ') : '';
    if (startInput) startInput.value = plan.startDate || '';
    if (targetInput) targetInput.value = plan.targetDate || '';

    const pr = plansComputeProgress(plan);
    const fill = document.getElementById('plansProgressFill');
    const txt = document.getElementById('plansProgressText');
    if (fill) fill.style.width = `${pr.pct}%`;
    if (txt) txt.innerText = `${pr.done}/${pr.total} • ${pr.pct}%`;

    const last = document.getElementById('plansLastUpdated');
    const t = plan.updatedAt ? new Date(plan.updatedAt) : null;
    if (last) last.innerText = t && !isNaN(t.getTime()) ? `Last updated ${t.toLocaleString()}` : '';

    const badges = document.getElementById('plansMetaBadges');
    if (badges) {
        const tagCount = Array.isArray(plan.tags) ? plan.tags.length : 0;
        badges.innerHTML = `
            <span class="plans-meta-badge"><i class="fas fa-tag"></i> ${escapeHtml(plan.category || 'Other')}</span>
            <span class="plans-meta-badge"><i class="fas fa-flag"></i> ${escapeHtml(plan.priority || 'Medium')}</span>
            <span class="plans-meta-badge"><i class="fas fa-circle-check"></i> ${escapeHtml(plan.status || 'Active')}</span>
            <span class="plans-meta-badge"><i class="fas fa-hashtag"></i> ${tagCount}</span>
        `;
    }

    plansRenderMilestones(plan);
    plansRenderSteps(plan);
    plansApplyReadOnlyState();
    plansApplyFocusModeState();
}

function plansRenderMilestones(plan) {
    const el = document.getElementById('plansMilestones');
    if (!el) return;
    const milestones = Array.isArray(plan?.milestones) ? plan.milestones : [];
    el.innerHTML = milestones.length
        ? milestones.map(m => {
            const id = (m?._id || m?.id || '').toString();
            const title = (m?.title || '').toString();
            return `<div class="plans-milestone" data-action="plans_remove_milestone" data-milestone="${id}">${escapeHtml(title)} <span style="float:right; cursor:pointer;" title="Remove">×</span></div>`;
        }).join('')
        : '';
}

function plansMilestoneOptions(plan, selectedId) {
    const milestones = Array.isArray(plan?.milestones) ? plan.milestones : [];
    const sid = (selectedId || '').toString();
    const opts = [`<option value="">No milestone</option>`]
        .concat(milestones.map(m => {
            const id = (m?._id || m?.id || '').toString();
            const t = (m?.title || '').toString();
            return `<option value="${escapeHtml(id)}" ${id === sid ? 'selected' : ''}>${escapeHtml(t)}</option>`;
        }));
    return opts.join('');
}

function plansRenderSteps(plan) {
    const el = document.getElementById('plansStepsList');
    if (!el) return;
    const steps = Array.isArray(plan?.steps) ? plan.steps : [];
    el.innerHTML = steps.length
        ? steps.map((s, idx) => {
            ...
            const status = (s?.status || 'Pending').toString();
            const msId = (s?.milestoneId || '').toString();
            const linked = (s?.linkedHabitName || '').toString();
            const upDisabled = idx === 0 ? 'disabled' : '';
            const downDisabled = idx === steps.length - 1 ? 'disabled' : '';
            const linkedLabel = linked ? `Linked: ${linked}` : 'Link to Habit';
            return `
                <div class="plans-step" data-step="${escapeAttr(id)}">
                    <div class="plans-step-top">
                        <input class="full-width-input plans-step-title" type="text" value="${escapeAttr(title)}" data-action="plans_step_title">
                        <div class="plans-step-controls">
                            <select class="full-width-select" data-action="plans_step_status">
                                <option value="Pending" ${status === 'Pending' ? 'selected' : ''}>Pending</option>
                                <option value="In Progress" ${status === 'In Progress' ? 'selected' : ''}>In Progress</option>
                                <option value="Completed" ${status === 'Completed' ? 'selected' : ''}>Completed</option>
                            </select>
                            <button class="btn-secondary" type="button" data-action="plans_step_up" ${upDisabled}><i class="fas fa-arrow-up"></i></button>
                            <button class="btn-secondary" type="button" data-action="plans_step_down" ${downDisabled}><i class="fas fa-arrow-down"></i></button>
                            <button class="btn-danger" type="button" data-action="plans_step_delete"><i class="fas fa-trash"></i></button>
                        </div>
                    </div>
                    <div style="margin-top:0.7rem; display:grid; grid-template-columns: 1fr; gap:0.7rem;">
                        <select class="full-width-select" data-action="plans_step_milestone"></select>
                        <textarea class="plans-step-notes" data-action="plans_step_notes" placeholder="Detailed notes / explanation">${escapeHtml(notes)}</textarea>
                        <button class="btn-secondary" type="button" data-action="plans_step_link_habit">${escapeHtml(linkedLabel)}</button>
                    </div>
                </div>
            `;
        }).join('')
        : '<div class="hint">No steps yet</div>';

    el.querySelectorAll('[data-action="plans_step_milestone"]').forEach(sel => {
        const stepEl = sel.closest('.plans-step');
        const stepId = stepEl?.dataset?.step || '';
        const step = (plan.steps || []).find(x => (x?._id || x?.id || '').toString() === stepId);
        const msId = (step?.milestoneId || '').toString();
        sel.innerHTML = plansMilestoneOptions(plan, msId);
    });
}

function plansApplyFocusModeState() {
    const layout = document.getElementById('plansLayout');
    if (!layout) return;
    layout.classList.toggle('plans-focus', !!plansState.focusMode);
}

function plansApplyReadOnlyState() {
    const editor = document.getElementById('plansEditor');
    if (editor) editor.contentEditable = plansState.readOnly ? 'false' : 'true';
    const title = document.getElementById('plansTitleInput');
    if (title) title.disabled = !!plansState.readOnly;
    const shortDesc = document.getElementById('plansShortDescInput');
    if (shortDesc) shortDesc.disabled = !!plansState.readOnly;
    ['plansEditCategory', 'plansEditPriority', 'plansEditStatus', 'plansEditTags', 'plansEditStartDate', 'plansEditTargetDate']
        .forEach(id => {
            const el = document.getElementById(id);
            if (el) el.disabled = !!plansState.readOnly;
        });
}

async function plansLoadAll() {
    if (plansIsAuthed()) {
        const list = await plansApi('/plans', { method: 'GET' });
        plansState.items = Array.isArray(list) ? list.map(plansNormalizeApiPlan) : [];
        return;
    }
    plansState.items = Array.isArray(appData?.plans) ? appData.plans : [];
}

async function plansCreateNew() {
    const base = {
        title: 'New Plan',
        shortDescription: '',
        category: 'Other',
    ...
    if (plansIsAuthed()) {
        const created = await plansApi('/plans', { method: 'POST', body: JSON.stringify(base) });
        const plan = plansNormalizeApiPlan(created);
        plansState.items = [plan, ...plansState.items];
        plansState.selectedId = plan.id;
        plansRenderList();
        plansRenderSelected();
        return;
    }
    ...
    const plan = { ...base, id, milestones: [], steps: [], updatedAt: now };
    appData.plans = [plan, ...(Array.isArray(appData.plans) ? appData.plans : [])];
    saveData();
    plansState.items = appData.plans;
    plansState.selectedId = id;
    plansRenderList();
    plansRenderSelected();
}

async function plansSaveSelectedPlan() {
    const plan = plansGetSelected();
    if (!plan) return;

    plansSetSaveIndicator('Saving…');
    ...
    try {
        if (plansIsAuthed()) {
            ...
            const saved = await plansApi(`/plans/${plan.id}`, { method: 'PUT', body: JSON.stringify(payload) });
            const normalized = plansNormalizeApiPlan(saved);
            plansState.items = plansState.items.map(p => (p.id === plan.id ? normalized : p));
            if (plansState.selectedId === plan.id) plansState.selectedId = normalized.id;
            plansRenderList();
            plansRenderSelected();
        } else {
            const now = new Date().toISOString();
            plan.updatedAt = now;
            if (!Array.isArray(appData.plans)) appData.plans = [];
            appData.plans = appData.plans.map(p => ((p?.id || '').toString() === plan.id ? plan : p));
            saveData();
            plansState.items = appData.plans;
            plansRenderList();
            plansRenderSelected();
        ...
        plansSetSaveIndicator('Error');
        console.error(e);
    }
}

async function plansDeleteSelected() {
    const plan = plansGetSelected();
    if (!plan) return;
    ...
    try {
        if (plansIsAuthed()) {
            await plansApi(`/plans/${plan.id}`, { method: 'DELETE' });
            plansState.items = plansState.items.filter(p => p.id !== plan.id);
        } else {
            appData.plans = (appData.plans || []).filter(p => (p?.id || '').toString() !== plan.id);
            saveData();
            plansState.items = appData.plans;
        }
        plansState.selectedId = plansState.items[0]?.id || null;
        plansRenderList();
        plansRenderSelected();
    } catch (e) {
        console.error(e);
...
async function plansAddStep() {
    const plan = plansGetSelected();
    if (!plan || plansState.readOnly) return;
    const title = prompt('Step title', 'New step');
    if (!title) return;

    try {
        if (plansIsAuthed()) {
            const saved = await plansApi(`/plans/${plan.id}/steps`, {
                method: 'POST',
                body: JSON.stringify({ title, notes: '', status: 'Pending', milestoneId: null, linkedHabitName: '' })
            });
            const normalized = plansNormalizeApiPlan(saved);
            plansState.items = plansState.items.map(p => (p.id === plan.id ? normalized : p));
            plansRenderList();
        ...
        plan.steps.push({ id, title, notes: '', status: 'Pending', milestoneId: '', linkedHabitName: '' });
        plansSchedule(`planSave:${plan.id}`, 500, plansSaveSelectedPlan);
        plansRenderList();
        plansRenderSelected();
    } catch (e) {
        console.error(e);
...
async function plansAddMilestone() {
    const plan = plansGetSelected();
    if (!plan || plansState.readOnly) return;
    const title = prompt('Milestone title', 'Milestone');
    if (!title) return;

    try {
        if (plansIsAuthed()) {
            const saved = await plansApi(`/plans/${plan.id}/milestones`, { method: 'POST', body: JSON.stringify({ title }) });
            const normalized = plansNormalizeApiPlan(saved);
            plansState.items = plansState.items.map(p => (p.id === plan.id ? normalized : p));
            plansRenderList();
            plansRenderSelected();
...
async function plansRemoveMilestone(milestoneId) {
    const plan = plansGetSelected();
    if (!plan || plansState.readOnly) return;
    ...
    try {
        if (plansIsAuthed()) {
            const saved = await plansApi(`/plans/${plan.id}/milestones/${mid}`, { method: 'DELETE' });
            const normalized = plansNormalizeApiPlan(saved);
            plansState.items = plansState.items.map(p => (p.id === plan.id ? normalized : p));
            plansRenderList();
            plansRenderSelected();
        ...
        plan.milestones = (plan.milestones || []).filter(m => (m?.id || m?._id || '').toString() !== mid);
        plan.steps = (plan.steps || []).map(s => {
            if ((s?.milestoneId || '').toString() === mid) s.milestoneId = '';
            return s;
        });
        plansSchedule(`planSave:${plan.id}`, 500, plansSaveSelectedPlan);
        plansRenderSelected();
...
async function plansUpdateStep(stepId, patch) {
    const plan = plansGetSelected();
    if (!plan || plansState.readOnly) return;
    ...
    const steps = Array.isArray(plan.steps) ? plan.steps : [];
    const idx = steps.findIndex(s => (s?._id || s?.id || '').toString() === sid);
    if (idx < 0) return;
    plan.steps[idx] = { ...steps[idx], ...patch };

    plansRenderList();
    plansRenderSelected();

    if (plansIsAuthed()) {
        plansSchedule(`stepSave:${plan.id}:${sid}`, 600, async () => {
            try {
                plansSetSaveIndicator('Saving…');
                const saved = await plansApi(`/plans/${plan.id}/steps/${sid}`, { method: 'PUT', body: JSON.stringify(patch) });
                const normalized = plansNormalizeApiPlan(saved);
                plansState.items = plansState.items.map(p => (p.id === plan.id ? normalized : p));
                plansSetSaveIndicator('Saved');
                plansRenderList();
    ...
    plansSchedule(`planSave:${plan.id}`, 600, plansSaveSelectedPlan);
}

async function plansDeleteStep(stepId) {
    const plan = plansGetSelected();
    if (!plan || plansState.readOnly) return;
    const sid = (stepId || '').toString();
    if (!sid) return;
    ...
    try {
        if (plansIsAuthed()) {
            const saved = await plansApi(`/plans/${plan.id}/steps/${sid}`, { method: 'DELETE' });
            const normalized = plansNormalizeApiPlan(saved);
            plansState.items = plansState.items.map(p => (p.id === plan.id ? normalized : p));
            plansRenderList();
            plansRenderSelected();
        ...
        plan.steps = (plan.steps || []).filter(s => (s?._id || s?.id || '').toString() !== sid);
        plansSchedule(`planSave:${plan.id}`, 500, plansSaveSelectedPlan);
        plansRenderSelected();
    } catch (e) {
        console.error(e);
...
async function plansMoveStep(stepId, dir) {
    const plan = plansGetSelected();
    if (!plan || plansState.readOnly) return;
    const sid = (stepId || '').toString();
    const steps = Array.isArray(plan.steps) ? plan.steps : [];
    ...
    if (plansIsAuthed()) {
        const orderedStepIds = plan.steps.map(s => (s?._id || s?.id || '').toString()).filter(Boolean);
        plansSchedule(`reorder:${plan.id}`, 400, async () => {
            try {
                plansSetSaveIndicator('Saving…');
                const saved = await plansApi(`/plans/${plan.id}/reorderSteps`, { method: 'PUT', body: JSON.stringify({ orderedStepIds }) });
                const normalized = plansNormalizeApiPlan(saved);
                plansState.items = plansState.items.map(p => (p.id === plan.id ? normalized : p));
                plansSetSaveIndicator('Saved');
                plansRenderList();
...
function plansExecCommand(cmd) {
    const editor = document.getElementById('plansEditor');
    if (!editor || plansState.readOnly) return;
    editor.focus();
    ...
    if (cmd === 'code') document.execCommand('insertHTML', false, '<pre><code>\n</code></pre>');
    if (cmd === 'checklist') document.execCommand('insertHTML', false, '<div>☐ </div>');
    plansOnEditorChanged();
}

function plansOnEditorChanged() {
    const plan = plansGetSelected();
    const editor = document.getElementById('plansEditor');
    if (!plan || !editor || plansState.readOnly) return;
    plan.contentHtml = editor.innerHTML || '';
    plansSchedule(`planSave:${plan.id}`, 800, plansSaveSelectedPlan);
    plansSetSaveIndicator('Saving…');
}

function plansExportText(plan) {
    const title = (plan?.title || 'Plan').toString();
    const tmp = document.createElement('div');
    tmp.innerHTML = plan?.contentHtml || '';
    const body = (tmp.innerText || '').trim();
    const steps = Array.isArray(plan?.steps) ? plan.steps : [];
    const stepText = steps.map((s, i) => `${i + 1}. [${s.status || 'Pending'}] ${(s.title || '').toString()}`).join('\n');
    return `${title}\n\n${body}\n\nSteps\n${stepText}\n`;
}

function plansDownload(filename, content, mime) {
...
function plansExportMarkdown(plan) {
    const title = (plan?.title || 'Plan').toString();
    const tmp = document.createElement('div');
    tmp.innerHTML = plan?.contentHtml || '';
    const body = (tmp.innerText || '').trim();
    const tags = Array.isArray(plan?.tags) && plan.tags.length ? `\n\nTags: ${plan.tags.map(t => `#${t}`).join(' ')}` : '';
    const meta = `Category: ${plan?.category || 'Other'}\nPriority: ${plan?.priority || 'Medium'}\nStatus: ${plan?.status || 'Active'}${plan?.startDate ? `\nStart: ${plan.startDate}` : ''}${plan?.targetDate ? `\nTarget: ${plan.targetDate}` : ''}${tags}`;
    const steps = Array.isArray(plan?.steps) ? plan.steps : [];
    const stepLines = steps.map(s => {
        const st = (s?.status || 'Pending').toString();
        const checkbox = st === 'Completed' ? '[x]' : '[ ]';
        return `- ${checkbox} ${(s?.title || '').toString()}`;
    }).join('\n');
    return `# ${title}\n\n${meta}\n\n## Notes\n\n${body}\n\n## Steps\n\n${stepLines}\n`;
}

function plansExportPdf(plan) {
    const w = window.open('', '_blank');
    if (!w) return;
    const pr = plansComputeProgress(plan);
    const tmp = document.createElement('div');
    tmp.innerHTML = plan?.contentHtml || '';
    const body = escapeHtml((tmp.innerText || '')).replaceAll('\n', '<br>');
    const steps = Array.isArray(plan?.steps) ? plan.steps : [];
    const stepsHtml = steps.map(s => `<li>${escapeHtml(s.title || '')} — ${escapeHtml(s.status || 'Pending')}</li>`).join('');
    const html = `
        <html>
            <head><title>Plan Export</title></head>
            <body>
                <h1>${escapeHtml(plan?.title || 'Plan')}</h1>
                <p>${escapeHtml(plan?.category || 'Other')} • ${escapeHtml(plan?.priority || 'Medium')} • ${escapeHtml(plan?.status || 'Active')}</p>
                <p>Progress: ${pr.done}/${pr.total} (${pr.pct}%)</p>
                <h2>Notes</h2>
                <div>${body}</div>
                <h2>Steps</h2>
                <ol>${stepsHtml}</ol>
...
function setupPlansEventListeners() {
    const root = document.body;
    if (root?.dataset?.plansBound === '1') return;
    if (root?.dataset) root.dataset.plansBound = '1';

    const bind = (id, ev, fn, flag) => {
        const el = document.getElementById(id);
        if (!el) return;
        const key = flag || `bound_${ev}`;
        if (el.dataset[key] === '1') return;
        el.dataset[key] = '1';
        el.addEventListener(ev, fn);
    };

    bind('plansNewBtn', 'click', () => {
        if (plansState.readOnly) return;
        plansCreateNew().catch(console.error);
    });
    bind('plansAddStepBtn', 'click', () => plansAddStep().catch(console.error));
    bind('plansAddMilestoneBtn', 'click', () => plansAddMilestone().catch(console.error));
    bind('plansReorderStepsBtn', 'click', () => {
        const steps = document.getElementById('plansStepsList');
        if (steps) steps.scrollIntoView({ behavior: 'smooth', block: 'start' });
    });
    bind('plansDeleteBtn', 'click', () => plansDeleteSelected().catch(console.error));
    bind('plansMarkCompletedBtn', 'click', () => {
        const plan = plansGetSelected();
        if (!plan || plansState.readOnly) return;
        plan.status = 'Completed';
        plansSchedule(`planSave:${plan.id}`, 300, plansSaveSelectedPlan);
        plansRenderList();
        plansRenderSelected();
    });
    bind('plansArchiveBtn', 'click', () => {
        const plan = plansGetSelected();
        if (!plan || plansState.readOnly) return;
    ...
    bind('plansFocusModeBtn', 'click', () => {
        plansState.focusMode = !plansState.focusMode;
        plansApplyFocusModeState();
    });
    bind('plansReadOnlyBtn', 'click', () => {
        plansState.readOnly = !plansState.readOnly;
        plansApplyReadOnlyState();
    });

    bind('plansExportPdfBtn', 'click', () => {
        const plan = plansGetSelected();
        if (plan) plansExportPdf(plan);
    });
    bind('plansExportMarkdownBtn', 'click', () => {
        const plan = plansGetSelected();
        if (!plan) return;
        const md = plansExportMarkdown(plan);
        plansDownload(`plan_${(plan.title || 'export').toString().slice(0, 40).replaceAll(' ', '_')}.md`, md, 'text/markdown');
    });
    bind('plansExportTextBtn', 'click', () => {
        const plan = plansGetSelected();
        if (!plan) return;
        const txt = plansExportText(plan);
        plansDownload(`plan_${(plan.title || 'export').toString().slice(0, 40).replaceAll(' ', '_')}.txt`, txt, 'text/plain');
    });

    const plansSection = document.getElementById('plansSection');
    if (plansSection && plansSection.dataset.boundDelegation !== '1') {
        plansSection.dataset.boundDelegation = '1';
        plansSection.addEventListener('click', (e) => {
            const cmdBtn = e.target?.closest?.('[data-plans-cmd]');
            if (cmdBtn) {
                plansExecCommand(cmdBtn.dataset.plansCmd);
                return;
            }

            const actionEl = e.target?.closest?.('[data-action]');
            const action = actionEl?.dataset?.action || '';
            if (action === 'plans_select') {
                const id = (actionEl.dataset.plan || '').toString();
                plansState.selectedId = id;
                plansRenderList();
                plansRenderSelected();
                return;
            }
            if (action === 'plans_remove_milestone') {
                plansRemoveMilestone(actionEl.dataset.milestone).catch(console.error);
                return;
            }
            if (actionEl?.closest?.('.plans-step')) {
                const stepEl = actionEl.closest('.plans-step');
                const sid = (stepEl?.dataset?.step || '').toString();
                if (action === 'plans_step_up') plansMoveStep(sid, -1).catch(console.error);
                if (action === 'plans_step_down') plansMoveStep(sid, 1).catch(console.error);
                if (action === 'plans_step_delete') plansDeleteStep(sid).catch(console.error);
                if (action === 'plans_step_link_habit') {
                    const plan = plansGetSelected();
                    if (!plan || plansState.readOnly) return;
                    const step = (plan.steps || []).find(s => (s?._id || s?.id || '').toString() === sid);
                    if (!step) return;
                    const habitName = prompt('Habit name for this step', (step?.title || '').toString());
                    if (!habitName) return;
                    addHabit(habitName, 'Plans');
                    plansUpdateStep(sid, { linkedHabitName: habitName }).catch(console.error);
                }
            }
        });
    }

    const bindFilter = (id) => bind(id, 'input', () => plansRenderList(), 'boundInput');
    ['plansSearchInput'].forEach(bindFilter);
    ['plansFilterCategory', 'plansFilterPriority', 'plansFilterStatus'].forEach(id => bind(id, 'change', () => plansRenderList(), 'boundChange'));

    bind('plansTitleInput', 'input', (e) => {
        const plan = plansGetSelected();
        if (!plan || plansState.readOnly) return;
        plan.title = (e.target?.value || '').toString().slice(0, 140);
        plansSetSaveIndicator('Saving…');
        plansSchedule(`planSave:${plan.id}`, 700, plansSaveSelectedPlan);
        plansRenderList();
    }, 'boundInput');

    bind('plansShortDescInput', 'input', (e) => {
        const plan = plansGetSelected();
        if (!plan || plansState.readOnly) return;
        plan.shortDescription = (e.target?.value || '').toString().slice(0, 500);
        plansSetSaveIndicator('Saving…');
        plansSchedule(`planSave:${plan.id}`, 800, plansSaveSelectedPlan);
        plansRenderList();
    }, 'boundInput');

    bind('plansEditor', 'input', () => plansOnEditorChanged(), 'boundInput');

    ['plansEditCategory', 'plansEditPriority', 'plansEditStatus', 'plansEditStartDate', 'plansEditTargetDate'].forEach(id => bind(id, 'change', (e) => {
        const plan = plansGetSelected();
        if (!plan || plansState.readOnly) return;
        const v = (e.target?.value || '').toString();
        if (id === 'plansEditCategory') plan.category = v;
        if (id === 'plansEditPriority') plan.priority = v;
        if (id === 'plansEditStatus') plan.status = v;
        if (id === 'plansEditStartDate') plan.startDate = v;
        if (id === 'plansEditTargetDate') plan.targetDate = v;
        plansSetSaveIndicator('Saving…');
        plansSchedule(`planSave:${plan.id}`, 700, plansSaveSelectedPlan);
        plansRenderList();
        plansRenderSelected();
    ...
    bind('plansEditTags', 'input', (e) => {
        const plan = plansGetSelected();
        if (!plan || plansState.readOnly) return;
        const raw = (e.target?.value || '').toString();
        plan.tags = raw.split(',').map(x => x.trim()).filter(Boolean).slice(0, 50);
        plansSetSaveIndicator('Saving…');
        plansSchedule(`planSave:${plan.id}`, 800, plansSaveSelectedPlan);
        plansRenderSelected();
    }, 'boundInput');

    if (plansSection && plansSection.dataset.boundInputDelegation !== '1') {
        plansSection.dataset.boundInputDelegation = '1';
        plansSection.addEventListener('input', (e) => {
            const actionEl = e.target?.closest?.('[data-action]');
            const action = actionEl?.dataset?.action || '';
            const stepEl = e.target?.closest?.('.plans-step');
            const sid = (stepEl?.dataset?.step || '').toString();
            if (!sid) return;

            if (action === 'plans_step_title') {
                plansUpdateStep(sid, { title: (e.target?.value || '').toString().slice(0, 140) }).catch(console.error);
            }
            if (action === 'plans_step_notes') {
                plansUpdateStep(sid, { notes: (e.target?.value || '').toString().slice(0, 5000) }).catch(console.error);
            }
        });

        plansSection.addEventListener('change', (e) => {
            const actionEl = e.target?.closest?.('[data-action]');
            const action = actionEl?.dataset?.action || '';
            const stepEl = e.target?.closest?.('.plans-step');
            const sid = (stepEl?.dataset?.step || '').toString();
            if (!sid) return;
            if (action === 'plans_step_status') {
                plansUpdateStep(sid, { status: (e.target?.value || 'Pending').toString() }).catch(console.error);
            }
            if (action === 'plans_step_milestone') {
                const v = (e.target?.value || '').toString();
                plansUpdateStep(sid, { milestoneId: v || null }).catch(console.error);
            }
        });
    }
}
...
async function renderPlans() {
    ...
    const key = plansIdentityKey();
    if (plansState.loadedKey !== key) {
        plansState.loadedKey = key;
        plansState.items = [];
        plansState.selectedId = null;
        plansState.focusMode = false;
        plansState.readOnly = false;
    }

    try {
        if (!plansState.items.length) {
            await plansLoadAll();
            const firstId = plansState.items[0]?.id || null;
            if (!plansState.selectedId && firstId) plansState.selectedId = firstId;
        }
    } catch (e) {
        console.error(e);
        plansSetSaveIndicator('Error');
...
async function handleLogin() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            sessionStorage.setItem(STORAGE_KEY_SESSION, data.user.name);
            location.reload();
        } else {
            err.innerText = data?.msg || (data?.errors && data.errors[0]?.msg) || text || "Login failed";
        }
    } catch (e) {
        console.error('Login error:', e);
        let errorMsg = "Cannot connect to server. ";
        
        // Check if accessing via file:// protocol (CORS issue)
        if (window.location.protocol === 'file:') {
            errorMsg += "You're opening the HTML file directly. Please access the app via http://localhost:5000 instead.";
            ...
            errorMsg += "Make sure the backend is running. Start it with: npm start or node server.js";
        } else {
            errorMsg += `Error: ${e.message || 'Unknown error'}. Check browser console for details.`;
        }
    ...
    try {
        ...
        if (res.ok) {
            if (!data || !data.token || !data.user || !data.user.id) {
                err.innerText = "Registration failed. Invalid server response.";
                return;
            }
            localStorage.setItem('token', data.token);
            localStorage.setItem('userId', data.user.id);
            ...
            err.innerText = data?.msg || (data?.errors && data.errors[0]?.msg) || text || "Registration failed";
        }
    } catch (e) {
        console.error('Register error:', e);
        if (e.message && e.message.includes('fetch')) {
            err.innerText = "Cannot connect to server. Make sure the backend is running on port 5000. Start it with: node server.js";
        } else {
            err.innerText = `Server Error: ${e.message || 'Unknown error'}. Check console for details.`;
        }
    }
}
    ...
    if (token) {
        try {
            ...
            if (res.ok) {
                ...
                newHabit.logs[dateStr] = {
                    status: 'not completed',
                    _id: apiHabit._id
                };
            } else if (res.status === 403) {
                const payload = await res.json().catch(() => ({}));
                if (payload && payload.upgradeSuggest) {
                    showUpgradePrompt(payload.message || 'Feature restricted on current plan');
                }
            }
        } catch (e) {
            console.error("Failed to sync new habit to backend", e);
        }
    }
...
async function deleteHabit(id) {
    if (confirm("Delete this habit and all its history?")) {
        ...
        // 1. Delete all logs from backend
        if (habit.logs) {
...
async function toggleHabitStatus(habitId, dateStr, status, opts) {
    ...
    const logEntry = habit.logs[dateStr];
    const token = localStorage.getItem('token');

    // Optimistic Update / Local Logic Prep
    let action = 'none'; // 'create', 'update', 'delete'
    let newLogData = null;

    if (logEntry && logEntry.status === status) {
        // Toggle OFF (Delete)
        action = 'delete';
    } else {
        // Toggle ON or Change Status
        action = logEntry ? 'update' : 'create';
    }
    ...
    try {
        if (token) {
            ...
            if (action === 'delete') {
                if (logEntry._id) {
                    await fetch(`${API_URL}/habits/${logEntry._id}`, {
                        method: 'DELETE',
                        headers: { 'Authorization': `Bearer ${token}` }
                    });
                }
        ...
        if (action === 'delete') {
            ...
            if (!hasToken && status === 'completed') addXP(-10);
        } else {
            const oldStatus = prevStatus;
            habit.logs[dateStr] = {
                status: status,
                note: habit.logs[dateStr]?.note || "",
                _id: newLogData?._id || habit.logs[dateStr]?._id // Store backend ID
            };
        ...
        if (currentUser) {
            if (!suppressFeed && status === 'completed' && prevStatus !== 'completed') {
                addFeedEvent(currentUser, 'habit_completed', { 
                    habitId: habit.id, 
                    name: habit.name, 
                    category: mapHabitCategory(habit.category), 
                    date: dateStr 
                }, ensureSocialUser(currentUser)?.profile?.privacy === 'private' ? 'followers' : 'public');
            }
            milestonesFromStreak(currentUser, appData);
            updateRankForUser(currentUser, appData, false);
            checkChallengeCompletionsForUser(currentUser);
        ...
        // Save local state (for non-synced data like XP)
        const userId = localStorage.getItem('userId');
        if (userId) {
            localStorage.setItem(`extra_data_${userId}`, JSON.stringify(appData));
        } else {
        ...
        console.error("Error syncing habit status:", err);
        alert("Failed to sync change. Please check your connection.");
    }
}

async function saveHabitNote(habitId, dateStr, note) {
    const habit = appData.habits.find(h => h.id === habitId);
    if (habit && habit.logs[dateStr]) {
        ...
        if (token && logEntry._id) {
            try {
                ...
                console.error("Failed to save note backend", e);
            }
        }
...
function updateUI() {
    ...
    if (planBadge) {
        const plan = appData.user.subscriptionPlan || 'free';
        planBadge.innerText = plan === 'pro' ? 'Pro' : 'Free';
    }
}
...
function renderCharts(viewMode = 'weekly') {
    ...
    const mainWrapper = mainCanvas.parentElement;
    const pieWrapper = pieCanvas.parentElement;
    if (mainWrapper) {
        mainCanvas.width = mainWrapper.clientWidth;
        mainCanvas.height = mainWrapper.clientHeight;
    }
    if (pieWrapper) {
        pieCanvas.width = pieWrapper.clientWidth;
        pieCanvas.height = pieWrapper.clientHeight;
    }
    ...
    if (currentUser) {
        profileContent.innerHTML = `
            ...
            <div class="profile-stats">
                ...
                <div class="p-stat">
                    <strong>${(appData.user.subscriptionPlan || 'free').toUpperCase()}</strong>
                    <span>Plan</span>
                </div>
            </div>
            <hr>
            ...
            <div class="form-group">
                <button class="btn-secondary full-width" onclick="showReferralCode()">
                    <i class="fas fa-share-alt"></i> Referral & Sharing
                </button>
            </div>
            <div class="form-group">
                <button class="btn-secondary full-width" onclick="requestAccountDeletion()">
...
function exportData() {
    const token = localStorage.getItem('token');
    if (token) {
        fetch(`${API_URL}/account/export?format=csv`, {
            headers: { 'Authorization': `Bearer ${token}` }
        }).then(async res => {
            if (!res.ok) return;
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = 'habitify_export.csv';
            document.body.appendChild(a);
            a.click();
            a.remove();
...
async function showReferralCode() {
    const token = localStorage.getItem('token');
    if (!token) {
        alert('Login to access referrals.');
        return;
    }
    try {
        ...
        prompt('Share this link with friends to earn rewards:', url);
    } catch (e) {}
}
...
async function requestAccountDeletion() {
    const token = localStorage.getItem('token');
    if (!token) return;
    if (!confirm('Request deletion of your account and data?')) return;
    try {
        ...
        if (res.ok) {
            alert('Deletion request submitted. We will process it soon.');
        }
    } catch (e) {}
}
